...
<template>
          ...
          <SidebarMenuItem v-for="item in favourites" :key="item.id" class="list-none w-full mb-1 last:mb-0">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent side="right" align="start" class="w-48">
                ...
                <DropdownMenuItem v-if="item.id" @click="deleteFavouriteById(item?.id)"
            ...
            <SidebarGroup
              ...
              <SidebarGroupLabel asChild>
                <CollapsibleTrigger
                  ...
                  <button v-if="collection.id" @click.stop="deleteCollectionById(collection.id)"
  ...
  <RecipeCard v-if="recipe" :recipe="recipe" :open="isRecipeModalOpen"
    @update:open="isRecipeModalOpen = $event; recipe = null" />

  <NewCollection :open="isCollectionModalOpen" @update:open="isCollectionModalOpen = $event" />

  <SelectCollection v-if="currentRecipe" v-model:open="selectCollectionOpen" :recipe-id="currentRecipe.recipeId"
    @collection-selected="handleCollectionSelected" />

  <SelectRecipe v-model:open="isRecipeSelectModalOpen" @recipe-selected="handleSelectRecipeUpdate" />
</template>
